...
export const getModelColumnsSelect = (id: Key, allModels: any[], level = 1) => {
  const select_data = allModels?.find((v) => v.id == id);
  //console.log(allModels, select_data);
  const fields: TreeData = select_data?.columns
    ?.map((v: any) => ({
  ...
  const guanlian: TreeData = select_data?.relations?.map((v: any) => ({
    ...
    value: v.name,
    children: getModelColumnsSelect(v.foreign_model_id, allModels, level + 1),
  }));
  return fields ? [...fields, ...(guanlian || [])] : [];
};

export const getModelById = (model_id: Key, models: any[]) => {
...
export const getModelColumns = (
  ...
  const foreignOptions = model?.columns?.map((v: any) => ({
    label: [v.title, v.name].join(' - '),
    value: v.name,
  }));
  if (!justTop) {
    model?.relations?.forEach((v: any) => {
      if (v.type == 'many') {
        if (v.with_count) {
          foreignOptions.push({
        ...
        if (v.with_sum) {
          v.with_sum.split(',').forEach((vs: any) => {
            foreignOptions.push({
